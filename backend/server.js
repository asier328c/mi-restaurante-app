...
db.exec(`CREATE TABLE IF NOT EXISTS items_pedido (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
const count = db.prepare("SELECT COUNT(*) as count FROM platos").get();
if (count.count === 0) {
    const insert = db.prepare("INSERT INTO platos (nombre, descripcion, precio, categoria) VALUES (?, ?, ?, ?)");
    const platos = [
...
app.get('/platos', async (req, res) => {
    try {
        ...
        res.status(500).json({ exito: false, mensaje: 'Error al cargar el menu' });
    }
});

app.post('/pedidos', async (req, res) => {
    const { cliente_nombre, cliente_telefono, cliente_direccion, items, metodo_pago, notas } = req.body;
    ...
    try {
        ...
        for (const item of items) {
            ...
            total += platoResult.rows[0].precio * item.cantidad;
        }
        ...
        res.status(201).json({
            ...
            pedido: { id: pedidoId, total: total, estado: 'pendiente' }
        });
    } catch (error) {
        console.error('Error:', error);
        res.status(500).json({ exito: false, mensaje: 'Error al procesar el pedido: ' + error.message });
    }
});
...
app.get('/pedidos', async (req, res) => {
    try {
        ...
        console.error('Error:', error);
        res.status(500).json({ exito: false, mensaje: 'Error al cargar los pedidos' });
    }
});
...
app.patch('/pedidos/:id', async (req, res) => {
    ...
    if (!estadosValidos.includes(estado)) {
        return res.status(400).json({ exito: false, mensaje: 'Estado no valido' });
    }
...
app.get('/estadisticas', async (req, res) => {
    try {
        ...
        const pedidosHoy = await dbAsync.query(
            `SELECT COUNT(*) as total, COALESCE(SUM(total), 0) as ingresos FROM pedidos WHERE DATE(creado_en) = ?`,
            [hoy]
        );
        const semana = await dbAsync.query(
            `SELECT COUNT(*) as total, COALESCE(SUM(total), 0) as ingresos FROM pedidos WHERE creado_en >= DATE('now', '-7 days')`
        );
        const topPlatos = await dbAsync.query(
            `SELECT p.nombre, SUM(ip.cantidad) as vendidos, SUM(ip.subtotal) as ingresos
             FROM items_pedido ip JOIN platos p ON ip.plato_id = p.id
        ...
        const porHora = await dbAsync.query(
            `SELECT strftime('%H', creado_en) as hora, COUNT(*) as cantidad
             FROM pedidos WHERE creado_en >= DATE('now', '-7 days') GROUP BY hora ORDER BY hora`
        );
        const porEstado = await dbAsync.query(
            `SELECT estado, COUNT(*) as cantidad FROM pedidos WHERE creado_en >= DATE('now', '-7 days') GROUP BY estado`
        );

        res.json({
            exito: true,
            datos: {
                hoy: { pedidos: pedidosHoy.rows[0].total, ingresos: pedidosHoy.rows[0].ingresos },
                semana: { pedidos: semana.rows[0].total, ingresos: semana.rows[0].ingresos },
        ...
        console.error('Error:', error);
        res.status(500).json({ exito: false, mensaje: 'Error al cargar estadisticas' });
    }
});
...
app.get('/clientes', async (req, res) => {
    try {
        const result = await dbAsync.query(
            `SELECT cliente_nombre as nombre, cliente_telefono as telefono, cliente_direccion as direccion,
                COUNT(*) as total_pedidos, COALESCE(SUM(total), 0) as total_gastado, MAX(creado_en) as ultimo_pedido
             FROM pedidos WHERE cliente_telefono IS NOT NULL AND cliente_telefono != ''
             GROUP BY cliente_nombre, cliente_telefono, cliente_direccion ORDER BY total_pedidos DESC`
        );
        res.json({ exito: true, datos: result.rows });
    } catch (error) {
        console.error('Error:', error);
        res.status(500).json({ exito: false, mensaje: 'Error al cargar clientes' });
    }
});
...
app.get('/pedidos/:id/detalles', async (req, res) => {
    const { id } = req.params;
    try {
        const pedidoResult = await dbAsync.query('SELECT * FROM pedidos WHERE id = ?', [id]);
        if (pedidoResult.rows.length === 0) {
            return res.status(404).json({ exito: false, mensaje: 'Pedido no encontrado' });
        }
        const itemsResult = await dbAsync.query(
            `SELECT ip.*, p.nombre, p.descripcion FROM items_pedido ip JOIN platos p ON ip.plato_id = p.id WHERE ip.pedido_id = ?`,
            [id]
        );
        res.json({ exito: true, datos: { pedido: pedidoResult.rows[0], items: itemsResult.rows } });
    } catch (error) {
        console.error('Error:', error);
        res.status(500).json({ exito: false, mensaje: 'Error al cargar detalles' });
    }
});
